...
- 네트워크 ACL의 기능
    - 상태 비저장, Stateless
    - 트래픽 허용 여부를 결정할 떄 가장 낮은 번호으 ㅣ규칙부터 순서대로 처리
- 보안그룹
    - 상태 저장, Stateful
    - 트래픽을 허용하기 전에 모든 규칙을 평가함
    - 인스턴스 수준에서 동작

### 26번
- 1년에 한번 24시간 동안 실행되는 무중단 워크로드에 대한 가장 비용 효율적인 EC2 요금 모델 = 온디맨드 인스턴스

### 27번
- AWS를 사용하여 총 소유비용을 줄이는 방법
    - 큰 자본 지출을 최소화함으로써
    - 구매 등의 투자 비용을 최소화

### 30번
-  AWS 계정 루트 계정 사용자 자격 증명이 필요한 경우
    - AWS 계정 닫기


### 31번
- 부하 분산(load balancer) = 트래픽을 분산하는 서비스
- 자동 장애 조치 = 시스템 문제 발생시 정상 시스템으로 전환하는 기능
- 라운드 로빈 = 순서대로 할당을 하는 스케줄링 방법
- Auto Scaling = 부하 변화에 따라 컴퓨팅 용량 조정

### 32번
- 애플리케이션이 중단 지속X, 한번에 몇주만 지속되는 일부 계절적 스파이크 포함, 예측가능한 사용량, 애플리케이션 수정 불가 등의 조건들에 가장 비용 효율적인 구매 옵션
    - 연중 예상 사용량에 대한 예약 인스턴스 구매 계절별 사용량이 온디맨드 요금으로 실행되도록 허용

### 33번
- AWS 계정 루트 계정 사용자 자격 증명이 필요한 경우
    - AWS Support 플랜 변경
    - IAM 사용자에게 전체 관리 권한 부여

### 36번
- AWS 클라우드의 장점
    - 사용자는 데이터 센터 유지 관리에 돈을 쓰지 않아도 된다 -> AWS에서 인프라 관리
    - 사용자는 리소스 용량에 대한 추측을 중지할 수 있다 -> 필요할 떄마다 확장 가능

### 38번
- 트래픽 증가 수용, 다운타임 발생 x 등의 요건을 만족하는 EC2 인스턴스 구매 옵션 = 온디맨드 인스턴스

### 40번
- 네트워크 접근 제어 목록(NACL) = 서브넷 수준의 방화벽
- 보안 그룹 = 인스턴스 수준의 방화벽, 인바운드 및 아웃바운드 액세스 제어
- AWS Trusted Advisor = 사용자의 AWS 환경을 조사하여 비용 절감, 시스템 성능 향상, 보안 결함 제거를 위한 권장 사항 제시
- 가상 프라이빗 게이트웨이 = 온프레미스와 연결하기 위한 VPC의 게이트웨이